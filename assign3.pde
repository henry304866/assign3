...
int countNeighborBombs(int col, int row) {
  ...
  for (int m = col - 1; m <= col + 1; m++)
  {
    for (int n = row - 1; n <= row + 1; n++)
    {
     ...
     {
        if (m < 0 || n < 0 || m >=4 || n >= 4){ // makes sure that it wont have an error for buttons next to the wall
            continue;
     
        } else if (slot[m][n]==SLOT_BOMB){
         count++;
        }
...
void setBombs() {
  ...
  // -------------- put your code here ---------
  // randomly set bombs
  int settingBombCount=bombCount;

  while (settingBombCount>0) {
